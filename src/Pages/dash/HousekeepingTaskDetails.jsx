...
const HousekeepingTaskDetails = ({ taskId }) => {
  ...
  const fetchTaskDetails = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:5000/api/housekeeping/tasks/${taskId}`, {
  ...
  const updateTaskStatus = async (newStatus) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`http://localhost:5000/api/housekeeping/tasks/${taskId}/status`, 
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` }}
      );
      fetchTaskDetails();
    ...
    <div className="p-4 bg-white rounded shadow">
      ...
      <div className="mb-6">
        <div className="flex items-center">
          <div className={`w-1/4 text-center ${task.status !== 'pending' ? 'opacity-50' : ''}`}>
            <div className={`h-8 w-8 rounded-full mx-auto flex items-center justify-center ${task.status === 'pending' ? 'bg-yellow-500 text-white' : 'bg-gray-200'}`}>1</div>
            <div className="mt-1 text-xs">Pending</div>
          </div>
          <div className="w-1/4 h-1 bg-gray-200"></div>
          <div className={`w-1/4 text-center ${task.status !== 'in-progress' ? 'opacity-50' : ''}`}>
            <div className={`h-8 w-8 rounded-full mx-auto flex items-center justify-center ${task.status === 'in-progress' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}>2</div>
            <div className="mt-1 text-xs">In Progress</div>
          </div>
          <div className="w-1/4 h-1 bg-gray-200"></div>
          <div className={`w-1/4 text-center ${task.status !== 'completed' && task.status !== 'verified' ? 'opacity-50' : ''}`}>
            <div className={`h-8 w-8 rounded-full mx-auto flex items-center justify-center ${task.status === 'completed' || task.status === 'verified' ? 'bg-green-500 text-white' : 'bg-gray-200'}`}>3</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <p><span className="font-semibold">Room:</span> {task.roomId?.title} (#{task.roomId?.room_number})</p>
          ...
          <p><span className="font-semibold">Type:</span> {task.cleaningType}</p>
          <p><span className="font-semibold">Priority:</span> {task.priority}</p>
        </div>
        <div>
          <p><span className="font-semibold">Assigned to:</span> {task.assignedTo?.username || 'Unassigned'}</p>
          <p><span className="font-semibold">Created:</span> {new Date(task.createdAt).toLocaleString()}</p>
          {task.startTime && <p><span className="font-semibold">Started:</span> {new Date(task.startTime).toLocaleString()}</p>}
          {task.endTime && <p><span className="font-semibold">Completed:</span> {new Date(task.endTime).toLocaleString()}</p>}
        </div>
      </div>
      ...
      {canUpdateStatus ? (
        <div className="mb-6">
          ...
          <div className="mb-3 p-3 bg-blue-50 text-blue-700 rounded border border-blue-200">
            ...
            <ol className="list-decimal ml-5 mt-1">
              ...
              <li>After cleaning is complete, upload "After Cleaning" photos</li>
        ...
        <div className="mb-6 p-3 bg-gray-50 text-gray-600 rounded border border-gray-200">
          ...
          {task.assignedTo && (
            <p className="mt-1">This task is assigned to: <span className="font-medium">{task.assignedTo.username}</span></p>
          )}
        </div>
      )}
