  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < imagesToUpload.length; i++) {
        ...
        const base64 = await fileToBase64(imagesToUpload[i]);
        base64Images.push({
          name: imagesToUpload[i].name,
          type: imagesToUpload[i].type,
          size: imagesToUpload[i].size,
          base64: base64
        });
      }
      
      // Send base64 images directly to backend
      const endpoint = `http://localhost:5000/api/housekeeping/tasks/${taskId}/images/${uploadType}/base64`;
      const response = await axios.post(endpoint, { images: base64Images }, {
